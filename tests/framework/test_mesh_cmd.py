...
def test_extract_registration_key_from_current_tailscale_path_url() -> None:
    ...
    assert mesh_cmd._extract_registration_key(f"https://vpn.example.com/register/{key}") == key


def test_extract_registration_key_returns_empty_for_unrelated_output() -> None:
    assert mesh_cmd._extract_registration_key("tailscale is already connected") == ""
